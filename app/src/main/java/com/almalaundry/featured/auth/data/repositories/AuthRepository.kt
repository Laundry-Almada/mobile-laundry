...
//                    return Result.failure(Exception("Gagal membuat laundry: ${laundryResponse.errorBody()?.string()}"))
//                }
//
...
////    }
//
//    private suspend fun handleRegisterResponse(response: Response<AuthResponse<AuthData>>): Result<AuthData> {
...
//                    if (!laundriesResponse.isSuccessful || laundriesResponse.body()?.data.isNullOrEmpty()) {
...
//    }
    ...
    suspend fun register(dto: RegisterDto): Result<AuthData> {
        return try {
            ...
            val laundryId = when (dto.role) {
                "Owner" -> dto.laundryId // LaundryId sudah dibuat sebelumnya
                "Staff" -> dto.selectedLaundry // Laundry dipilih dari list yang sudah diambil
                else -> null
            }
